...
async def main():
    parser = argparse.ArgumentParser(
        description='A modern and intelligent approach to SNMP hacking')

    module_group = parser.add_argument_group('Modules')
    # module_group.add_argument(
    #     '--scan', action='store_true', help='Scan for SNMP services')
    module_group.add_argument(
        '-c',
        '--community',
        type=str,
        help='Login with a provided community string or line-delimited file')
    module_group.add_argument(
        '-u',
        '--username',
        type=str,
        help='Login with a username or line-delimited file')
    module_group.add_argument(
        '-p',
        '--password',
        type=str,
        help='Login with a password or line-delimited file')
    # Is used for auth and priv passwords/keys
    module_group.add_argument(
        '--bulkwalk',
        action='store_true',
        help='Collect as much information as possible')
    # module_group.add_argument(
    #     '--write',
    #     action='store_true',
    #     help='Check if write access is possible')
    module_group.add_argument(
        '--all',
        action='store_true',
        help=('CAUTION: Use all above modules and default dictionaries '
              'unless provided'))
    ...
    io_group.add_argument(
        ...
        '--target',
        type=str,
        help=('Singular hostname or IPv4/IPv6 address or file containing '
              'line-delimited targets'))
    io_group.add_argument(
        '-pt',
        '--port',
        default=161,
        help='Target port/range (e.g., 161 or 161,162 or 160-165)')
    io_group.add_argument(
        '-i',
        '--interface',
        type=str,
        help='Specify network interface (e.g., eth0, Ethernet0)')
    io_group.add_argument(
        '-eid',
        '--engine-id',
        type=str,
        help=('Specify a hex agent engine ID (e.g., '
              '0x80000000011234567890abcdef)'))
    io_group.add_argument(
        '-o',
        '--output',
        type=str,
        default='SNMPede_',
        help='CSV prepended output filename/path')
    io_group.add_argument(
        '-l',
        '--log',
        type=str,
        default='SNMPede_Log.txt',
        help='Debug level 2 text output file')
    io_group.add_argument(
        '-d',
        '--debug',
        type=int,
    ...
    io_group.add_argument(
        ...
        type=float,
        default=0.3,
        help='Seconds delay between each request')
    io_group.add_argument(
        '-or',
        '--oid-read',
        type=str,
        default='1.3.6.1.2.1.1.1.0',
        help='OID the Login module will read (default is sysDescr.0)')
    io_group.add_argument(
        '-tk',
        '--tasks',
        type=int,
    ...
    # io_group.add_argument(
    #     '-cb', '--combo', help=('File containing line-delimited combos '
    #                            '(host,port,username,password/string)'))
    # io_group.add_argument(
    #     '-ow', '--oid-write', type=str, default='', help='')
...
▐█▄▪▐███▐█▌██ ██▌▐█▌▐█▪·•▐█▄▄▌██. ██ ▐█▄▄▌
 ▀▀▀▀ ▀▀ █▪▀▀  █▪▀▀▀.▀    ▀▀▀ ▀▀▀▀▀•  ▀▀▀
    ...
    (config.ARGTIMEOUT, config.ARGRETRIES, config.ARGDELAY,
     config.ARGDEBUG, config.OID_READ) = (
        args.timeout,
        args.retries,
        args.delay,
        args.debug,
    ...
    if not args.target:
        ...
        # If -t is an existing file path, treat it as a targets file and
        # convert to a list.
        # Otherwise, treat -t as a literal IPv4/IPv6/FQDN target (and let
        # resolution validate it)
        ...
        if not targets:
            print(
                "[e] Target list is empty (targets file may be empty or "
                "unreadable)."
            )
            quit()
    ...
    resolved_targets = [
        ...
        if t and isinstance(t, (tuple, list)) and len(t) >= 3
        and t[1] is not None and t[2] in ("v4", "v6")
    ]

    if not resolved_targets:
        print(f"[e] No valid targets were resolved from: {args.target}")
        print(
            "[i] If you intended a targets file, verify the file exists. "
            "Otherwise provide a valid IPv4/IPv6 address or resolvable FQDN."
        )
        quit()
    ...
    if args.interface:
        ...
        # Check if interface has appropriate IP version(s) given the target(s)
        # provided
        ...
        if need_v4 and not iface_has_v4:
            print(
                f"[e] IPv4 targets were provided, but interface '{iface_name}'"
        ...
        if need_v6 and not iface_has_v6:
            print(
                f"[e] IPv6 targets were provided, but interface '{iface_name}'"
        ...
        for a in iface_addrs:
            ...
            elif (need_v6 and a.family == AF_INET6 and
                  not config.INTERFACE_ADDR6):
                config.INTERFACE_ADDR6 = a.address

        if (need_v4 and not config.INTERFACE_ADDR4) or (
                need_v6 and not config.INTERFACE_ADDR6):
            print(
                f"[e] Interface '{iface_name}' was found, but a required IP "
    ...
    if args.password or args.all:
        ...
        # SNMP v3 does not allow less than 8 character passwords. Error if we
        # have any
        if any(len(password) < 8 for password in passwords):
            print(
                "[e] SNMP v3 does not allow passwords less than 8 characters. "
                "Please adjust."
            )
            quit()
    ...
    #                          'Host,Protocol,Port,Service,Status\n')
    ...
    # If we are not spraying, but instead just checking authentication, let's
    # clarify that:
    ...
    if args.community:
        ...
        try:
            ...
            print(
                f"[e] A v1 community string {intention.lower()}ing error "
                f"occurred: {e}"
            )
            quit()
        ...
        try:
            ...
            print(
                f"[e] A v2c community string {intention.lower()}ing error "
                f"occurred: {e}"
            )
            quit()
    ...
    if args.username:
        ...
        instances = get_instances_with_attribute(
            Target_instances, 'Access', False)
        if instances:
            if config.ARGDEBUG >= 1:
                print(f"[d] Using the relevant ({len(instances)}) instances:")
            for id, instance in enumerate(Target_instances):
                tasks.append(
                    asyncio.create_task(
                        snmp_v3_get_multi(
        ...
        if len(
            get_instances_with_attribute(
                Target_instances,
                'Username')) == 0:
            print('[i] No SNMP v3 username(s) found.')
            if config.ARGDEBUG:
                print(
                    '[d] As no usernames were found, skipping AuthNoPriv and '
                    'AuthPriv spraying.'
                )

        print('[-] Writing results...\n')

        if (len(get_instances_with_attribute(Target_instances,
                                             'CommunityString')) == 0 and
                len(get_instances_with_attribute(Target_instances,
                                                 'Username')) == 0):
            quit()
    ...
    if args.password:
        ...
        if instances:
            ...
            try:
                ...
                msg = (
                    f"[e] An AuthNoPriv {intention.lower()}ing error occurred"
                )
                print(f"{msg}: {e}")
                quit()
            ...
            if len(
                ...
                if config.ARGDEBUG:
                    print(
                        '[d] As no AuthNoPriv passwords were found, skipping '
                        'AuthPriv spraying.'
                    )

            print('[-] Writing results...\n')

            if (len(Target_instances) == 0 and
                    len(get_instances_with_attribute(Target_instances,
                                                     'AuthPwd')) == 0):
                quit()
        ...
        if instances:
                ...
                tasks.append(
                    asyncio.create_task(
                        snmp_v3_get_multi(
                            ...
                            instance.Port,
                            instance.Username,
                            authpasswords=instance.AuthPwd,
                            authprotocols=instance.AuthProto,
                            privpasswords=passwords,
            ...
            try:
                ...
                print(
                    f"[e] An AuthPriv {intention.lower()}ing error occurred: "
                    f"{e}"
                )
                quit()
    ...
    if args.bulkwalk or args.all:
        ...
        if instances:
            print(
                '[i] BulkWalking v2c instances (Note: Skipping v1 as it '
                'is not supported)...'
            )
            if config.ARGDEBUG >= 1:
                print(
                    f"[d] Using the relevant ({len(instances)}) v1/2c "
                    "instances:"
                )
            for id, instance in enumerate(instances):
            ...
            print(
                '[d] As no community strings were found, skipping applicable '
                'BulkWalk.'
            )
        ...
        if instances:
            ...
            if config.ARGDEBUG >= 1:
                print(
                    f"[d] Using the relevant ({len(instances)}) v3 instances:")
            for id, instance in enumerate(instances):
                if config.ARGDEBUG >= 1:
                    print(instance)
                tasks.append(
